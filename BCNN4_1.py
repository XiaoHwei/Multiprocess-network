...
def cnnnet(x, keep_prob, num_classes):
    ...
    with tf.name_scope('flattened4') as scope:
        conv_t=tf.transpose(pool2,perm=[0,3,1,2])
        conv_t = tf.reshape(conv_t, [-1, 128, 15*15])
    ...
    with tf.name_scope('fc5') as scope:
        weights1 = tf.Variable(tf.truncated_normal([128 * 128, 512],
                                                    dtype=tf.float32,
                                                    stddev=1e-1), name='weights')
...
def picture(acc, loss, path):
    ...
    # 保存
    plt.savefig(path + "/BCNN4_accuracy.png")
    fig = plt.figure(figsize=(15, 10), dpi=80)
    # 绘制图形
    plt.plot(loss[1:])
    plt.xticks(fontproperties='Times New Roman', size=15)
    ...
    plt.savefig(path + "/BCNN4_loss.png")
    plt.show()
    return None
...
with tf.Session() as sess:
    ...
    # 保存模型
    path = "./picture/BCNN4_1/train"
    picture(train_ac, train_ls, path)
    path1 = "./picture/BCNN4_1/test"
    picture(test_ac, test_ac, path1)
    saver.save(sess, "./model/BCNN4_1/BCNN4_1_model")