...
def cnnnet(x, keep_prob, num_classes):
    ...
    with tf.name_scope('fc5') as scope:
        weights1 = tf.Variable(tf.truncated_normal([128 * 128, 1024],
                                                    dtype=tf.float32,
                                                    stddev=1e-1), name='weights')
    ...
    with tf.name_scope('fc6') as scope:
        weights3 = tf.Variable(tf.truncated_normal([1024, num_classes],
                                                       dtype=tf.float32,
                                                       stddev=1e-1), name='weights')
...
with tf.Session() as sess:
    ...
    # 保存模型
    path = "./picture/BCNN4/train"
    picture(train_ac, train_ls, path)
    path1 = "./picture/BCNN4/test"
    picture(test_ac, test_ac, path1)
    saver.save(sess, "./model/BCNN4/BCNN4_model")