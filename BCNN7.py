...
def cnnnet(x, keep_prob, num_classes):
    ...
    with tf.name_scope('fc6') as scope:
        weights1 = tf.Variable(tf.truncated_normal([256 * 256 , 1024],
                                                    dtype=tf.float32,
                                                    stddev=1e-1), name='weights')
    ...
    with tf.name_scope('fc7') as scope:
        weights1 = tf.Variable(tf.truncated_normal([1024 ,6],
                                                    dtype=tf.float32,
                                                    stddev=1e-1), name='weights')
...
train_image_index = np.arange(
        train_image_num)  # arange(start，stop, step, dtype=None)根据start与stop指定的范围以及step设定的步长，生成一个 ndarray。
...
fc7 = cnnnet(x, keep_prob, num_classes)
...
with tf.name_scope('loss'):
    ...
    loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=fc7, labels=y_))+reg_term
with tf.name_scope('optimizer'):
    optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate)
    train_op = optimizer.minimize(loss_op)
# accuracy
with tf.name_scope("accuracy_1"):
    correct_pred = tf.equal(tf.argmax(fc7, 1), tf.argmax(y_, 1))
    accuracy = tf.reduce_mean(tf.cast(correct_pred, tf.float32))
init = tf.global_variables_initializer()

# Tensorboard
filewriter_path = './event/BCNN7'
...
with tf.Session() as sess:
    ...
    for i in range(train_num):
        ...
        for train_data_batch, train_label_batch in get_batch(train_data, train_label, batch_size):
            _, err, acc= sess.run([train_op, loss_op, accuracy],feed_dict={x: train_data_batch, y_: train_label_batch, keep_prob: 0.4})
            train_loss += err
            train_acc += acc
            batch_num += 1
            j = j + 1
            result = sess.run(merged_summary,feed_dict={x: train_data_batch, y_: train_label_batch, keep_prob: 0.4})
    ...
    # 保存模型
    path = "./picture/BCNN7/train"
    picture(train_ac, train_ls, path)
    saver.save(sess, "./model/BCNN7/BCNN7_model")