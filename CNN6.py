...
def alexnet(x, keep_prob, num_classes):
    ...
    with tf.name_scope('flattened4') as scope:
        flattened = tf.reshape(pool4, shape=[-1, 7 * 7 * 256], name="flatten")

    # fc4
    with tf.name_scope('fc5') as scope:
        weights1 = tf.Variable(tf.truncated_normal([7 * 7 * 256, 512],
                                                    dtype=tf.float32,
                                                    stddev=1e-1), name='weights')
        biases = tf.Variable(tf.constant(0.0, shape=[512], dtype=tf.float32),
                                 trainable=True, name='biases')
        bias = tf.nn.xw_plus_b(flattened, weights1, biases)
        fc4 = tf.nn.relu(bias, name='fc5')
        tf.add_to_collection(tf.GraphKeys.WEIGHTS, weights1)
    # dropout4
    with tf.name_scope('dropout5') as scope:
        dropout5 = tf.nn.dropout(fc4, keep_prob)
    # fc6
    with tf.name_scope('fc6') as scope:
        weights2 = tf.Variable(tf.truncated_normal([512, num_classes],
                                                       dtype=tf.float32,
                                                       stddev=1e-1), name='weights')
        biases = tf.Variable(tf.constant(0.0, shape=[num_classes], dtype=tf.float32),
                                 trainable=True, name='biases')
        fc6 = tf.nn.xw_plus_b(dropout5, weights2, biases, name='fc6')
...
def picture(acc, loss, path):
    ...
    # 保存
    plt.savefig(path + "/CNN6Net_accuracy.png")
    fig = plt.figure(figsize=(15, 10), dpi=80)
    # 绘制图形
    plt.plot(loss[1:])
    plt.xticks(fontproperties='Times New Roman', size=15)
    ...
    plt.savefig(path + "/CNN6Net_loss.png")
    plt.show()
    return None
...
fc6 = alexnet(x, keep_prob, num_classes)

# loss
with tf.name_scope('loss'):
    regularize = tf.contrib.layers.l2_regularizer(scale=lamda)
    reg_term = tf.contrib.layers.apply_regularization(regularize)
    loss_op = (tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=fc6, labels=y_)) + reg_term)
# optimizer
...
with tf.name_scope("accuracy_1"):
    correct_pred = tf.equal(tf.argmax(fc6, 1), tf.argmax(y_, 1))
    accuracy = tf.reduce_mean(tf.cast(correct_pred, tf.float32))

init = tf.global_variables_initializer()

# Tensorboard
filewriter_path = './event/CNN6'
...
with tf.Session() as sess:
    ...
    # 保存模型
    path = "./picture/CNN6/train"
    path1 = "./picture/CNN6/test"
    picture(train_ac, train_ls, path)
    picture(test_ac, test_ls, path1)
    saver.save(sess, "./model/CNN6/CNN6_model")
